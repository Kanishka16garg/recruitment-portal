    ...
    try {
      ...
      const link =
        queryParams.length > 0
          ? `${API_ENDPOINTS.JOB_GET_ALL}?${queryParams.join("&")}`
          : API_ENDPOINTS.JOB_GET_ALL;
...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${API_ENDPOINTS.JOB_GET_SINGLE}/${jobId}`,
...
export const postJob = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(API_ENDPOINTS.JOB_POST, data, {
      withCredentials: true,
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(API_ENDPOINTS.JOB_GET_MY_JOBS, {
      withCredentials: true,
    });
...
export const deleteJob = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(`${API_ENDPOINTS.JOB_DELETE}/${id}`, {
      withCredentials: true,
    });
